...
export const IMAGE_SLOTS: PageGroup[] = [
  ...
  },
  {
    ...
    slots: [
      ...
      { kind: "site", key: "sale_banner_1_image_mobile", label: "Banner de ofertas 1 — versión móvil",
        where: "Reemplaza al banner 1 en teléfono. Si lo dejas vacío se usa el horizontal.",
        spec: "Vertical, ~1080 × 1350 px (4:5). El banner horizontal en un teléfono queda como una franja muy fina." },
      { kind: "site", key: "sale_banner_2_image", label: "Banner de ofertas 2",
        where: "Segundo banner del slider de ofertas.",
        spec: "Horizontal y bajo, ~2400 × 600 px." },
      { kind: "site", key: "sale_banner_2_image_mobile", label: "Banner de ofertas 2 — versión móvil",
        where: "Reemplaza al banner 2 en teléfono. Si lo dejas vacío se usa el horizontal.",
        spec: "Vertical, ~1080 × 1350 px (4:5). El banner horizontal en un teléfono queda como una franja muy fina." },
      { kind: "site", key: "sale_banner_3_image", label: "Banner de ofertas 3",
        where: "Tercer banner del slider de ofertas.",
        spec: "Horizontal y bajo, ~2400 × 600 px." },
      { kind: "site", key: "sale_banner_3_image_mobile", label: "Banner de ofertas 3 — versión móvil",
        where: "Reemplaza al banner 3 en teléfono. Si lo dejas vacío se usa el horizontal.",
        spec: "Vertical, ~1080 × 1350 px (4:5). El banner horizontal en un teléfono queda como una franja muy fina." },
  ...
  },
  {
    ...
    slots: [
      ...
      { kind: "site", key: "wf_raid_jefe", label: "Raid — imagen del jefe",
        where: "Sección «El Guardián del Portal» de la landing: el jefe que todos atacan.",
        spec: "PNG con fondo transparente, cuadrado o vertical, ~1000 × 1000 px. Si lo dejas vacío se usa una silueta de sombra." },
      { kind: "site", key: "wf_raid_recompensa_img", label: "Raid — recompensa (la pieza del sorteo)",
        where: "Se revela con una animación cuando la comunidad derrota al jefe del raid, y queda visible en «Recompensa final».",
        spec: "PNG con fondo transparente, vertical, ~1200 × 1600 px. La pieza que se sorteará entre los Rango S." },
      { kind: "site", key: "email_logo_url", label: "Logo para correos (PNG)",
        where: "Cabecera de todos los correos que envía la tienda (pedidos, acceso, misiones, raid).",
        spec: "PNG con fondo transparente, horizontal, ~600 × 120 px. No uses SVG: Gmail no lo muestra. Si lo dejas vacío se usa el logo de la tienda o el nombre escrito." },
      { kind: "site", key: "icono_oficial", label: "Ícono oficial de la marca",
        where: "La calavera oficial: sello de la invitación a la rueda de prensa (/rueda-de-prensa).",
        spec: "PNG con fondo transparente, cuadrado, ~600 × 600 px. También sirve SVG." },
      { kind: "site", key: "rp_fondo", label: "Rueda de prensa — fondo",
        where: "Fondo de la portada de /rueda-de-prensa, detrás del título. Se ve tenue, con humo rojo encima.",
        spec: "JPG o WebP oscuro, vertical, ~1080 × 1600 px (por ejemplo, el arte de fondo de la invitación)." },
      { kind: "site", key: "wf_lista_video", label: "Lista de acceso — video de fondo",
        where: "Detrás del formulario donde dejan el correo, al final de la landing.",
        spec: "MP4 o WebM horizontal, corto y en bucle. Se ve bajo un velo oscuro." },
      { kind: "site", key: "wf_lista_image", label: "Lista de acceso — imagen de fondo",
        where: "Misma sección. Sirve de respaldo mientras carga el video, o sola si no hay video.",
        spec: "Horizontal, ~1920 × 1080 px." },
      { kind: "site", key: "wf_mundo_bg", label: "Fondo de «El mundo ha cambiado»",
        where: "Detrás de las tarjetas de «La escala de lo que viene».",
        spec: "Horizontal, ~1920 × 1080 px. Va bajo un velo oscuro y se funde arriba y abajo, así que conviene una imagen con el motivo en el centro." },
      { kind: "site", key: "wf_premio_image", label: "El premio",
        where: "Sección de la recompensa de rango S.",
        spec: "Vertical u horizontal. Se muestra muy oscurecida a propósito, para insinuar la pieza sin revelarla." },
      { kind: "site", key: "wf_invitado_1_foto", label: "Invitado 1 — foto",
        where: "Sección de invitados de la landing del evento.",
        spec: "Cuadrada, ~800 × 800 px. Si la dejas vacía sale el candado de «Próximamente»." },
      { kind: "site", key: "wf_invitado_2_foto", label: "Invitado 2 — foto",
        where: "Sección de invitados de la landing del evento.",
        spec: "Cuadrada, ~800 × 800 px. Si la dejas vacía sale el candado de «Próximamente»." },
      { kind: "site", key: "wf_invitado_3_foto", label: "Invitado 3 — foto",
        where: "Sección de invitados de la landing del evento.",
        spec: "Cuadrada, ~800 × 800 px. Si la dejas vacía sale el candado de «Próximamente»." },
    ],
  },
  ...
  },
  {
    ...
    slots: [
      { kind: "site", key: "aliado_1_logo", label: "Aliado 1 — logo",
        where: "Carrusel de aliados en la página de inicio.",
        spec: "PNG con fondo transparente, ~400 × 200 px. Se muestra en gris y se colorea al pasar por encima." },
      { kind: "site", key: "aliado_2_logo", label: "Aliado 2 — logo",
        where: "Carrusel de aliados en la página de inicio.",
        spec: "PNG con fondo transparente, ~400 × 200 px. Se muestra en gris y se colorea al pasar por encima." },
      { kind: "site", key: "aliado_3_logo", label: "Aliado 3 — logo",
        where: "Carrusel de aliados en la página de inicio.",
        spec: "PNG con fondo transparente, ~400 × 200 px. Se muestra en gris y se colorea al pasar por encima." },
      { kind: "site", key: "aliado_4_logo", label: "Aliado 4 — logo",
        where: "Carrusel de aliados en la página de inicio.",
        spec: "PNG con fondo transparente, ~400 × 200 px. Se muestra en gris y se colorea al pasar por encima." },
      { kind: "site", key: "aliado_5_logo", label: "Aliado 5 — logo",
        where: "Carrusel de aliados en la página de inicio.",
        spec: "PNG con fondo transparente, ~400 × 200 px. Se muestra en gris y se colorea al pasar por encima." },
      { kind: "site", key: "aliado_6_logo", label: "Aliado 6 — logo",
        where: "Carrusel de aliados en la página de inicio.",
        spec: "PNG con fondo transparente, ~400 × 200 px. Se muestra en gris y se colorea al pasar por encima." },
      { kind: "site", key: "aliado_7_logo", label: "Aliado 7 — logo",
        where: "Carrusel de aliados en la página de inicio.",
        spec: "PNG con fondo transparente, ~400 × 200 px. Se muestra en gris y se colorea al pasar por encima." },
      { kind: "site", key: "aliado_8_logo", label: "Aliado 8 — logo",
        where: "Carrusel de aliados en la página de inicio.",
        spec: "PNG con fondo transparente, ~400 × 200 px. Se muestra en gris y se colorea al pasar por encima." },
    ],
  },
  ...
  },
  {
    ...
    slots: [
      ...
      { kind: "site", key: "invitacion_media_image", label: "Personaje del cierre",
        where: "Encima de «Tu personaje. Tu historia. Tu mundo.», al cierre de la invitación.",
        spec: "Vertical, ~600 × 900 px. Pensada para un personaje recortado: PNG con fondo transparente es lo que mejor queda." },
      { kind: "site", key: "invitacion_previa_image", label: "Imagen antes del cierre",
        where: "Justo encima de «El próximo mundo está por abrirse».",
        spec: "Horizontal, ~1800 × 900 px. Funciona bien una foto de grupo o de evento." },
