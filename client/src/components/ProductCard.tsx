    ...
    <Link href={`/product/${slug}`}>
      <div className="group cursor-pointer rounded-2xl overflow-hidden bg-white border border-[#ebebeb] shadow-sm hover:shadow-md transition-shadow">
        ...
        <div className="relative aspect-square overflow-hidden bg-[#f5f5f5]">
          ...
          {/* Discount badge */}
          {hasDiscount && (
            <span className="absolute top-3 left-3 bg-[#1a1a1a] text-white text-[10px] font-bold px-2 py-1 rounded-full leading-none">
              -{discountPct}%
            </span>
          )}
          ...
          {!isOutOfStock && (
            <button
              ...
              disabled={isLoading}
              className="absolute bottom-3 left-3 right-3 bg-[#1a1a1a] text-white text-[13px] font-semibold py-2.5 rounded-xl opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 transition-all duration-200 flex items-center justify-center gap-1.5 disabled:opacity-50"
            >
              <ShoppingBag size={13} />
              {t.catalog.addToCart}
            </button>
